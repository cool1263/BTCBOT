...
const NetworkUtils = {
  ...
  async fetchWithRetry(url, options = {}, retries = CONSTANTS.MAX_RETRIES) {
    let lastError;
...
const WebSocketUtils = {
  safeClose(socket) {
    try {
      if (socket.readyState === CONSTANTS.WS_STATES.OPEN || 
          socket.readyState === CONSTANTS.WS_STATES.CLOSING) {
        socket.close();
      }
    } catch (error) {
      ErrorHandler.log(error, 'WebSocket close error');
    }
  },

  createReadableStream(webSocket, earlyDataHeader, log) {
...
setInterval(() => {
  ...
  for (const [key, value] of requestCache.entries()) {
    if (now - value.timestamp > CONSTANTS.CACHE_TTL) {
...
const ConnectionManager = {
  async createSocket(address, port, options = {}) {
  ...
  async handleSocketData(socket, handler) {
...
async function handleDNSQuery(udpChunk, webSocket, 维列斯ResponseHeader, log) {
  ...
  const dnsPort = CONSTANTS.PORTS.DNS;

  try {
    const tcpSocket = await ConnectionManager.createSocket(dnsServer, dnsPort);
    if (!tcpSocket) {
      throw new Error('Failed to create DNS socket connection');
    }
    ...
    await tcpSocket.readable.pipeTo(new WritableStream({
      async write(chunk) {
        if (webSocket.readyState === CONSTANTS.WS_STATES.OPEN) {
      ...
      abort(reason) {
        ErrorHandler.log(reason, `DNS 服务器(${dnsServer}) TCP 连接异常中断`);
      },
    }));
  } catch (error) {
    ErrorHandler.log(error, 'handleDNSQuery 函数发生异常');
  }
}
...
async function socks5Connect(addressType, addressRemote, portRemote, log) {
  ...
  try {
    const socket = await ConnectionManager.createSocket(hostname, port);
    if (!socket) {
    ...
    if (!res || res[0] !== 0x05) {
      throw new Error(`SOCKS5 服务器版本错误: 收到 ${res ? res[0] : 'undefined'}，期望是 5`);
    }
    ...
    // 发送连接请求
    const connectRequest = buildSocks5ConnectRequest(addressType, addressRemote, portRemote);
    await writer.write(connectRequest);
    
    res = (await reader.read()).value;
    if (!res || res[1] !== 0x00) {
      throw new Error('SOCKS5 连接建立失败');
    }
    ...
    ErrorHandler.log(error, 'SOCKS5 连接失败');
    throw error;
  }
}

// 辅助函数：构建SOCKS5连接请求
function buildSocks5ConnectRequest(addressType, address, port) {
  const encoder = new TextEncoder();
  let addrBytes;
  let atyp;
  ...
  switch (addressType) {
    ...
    case 2: // 域名
      ...
      addrBytes = new Uint8Array(encoded.length + 1);
      addrBytes[0] = encoded.length;
      addrBytes.set(encoded, 1);
      break;
    case 3: // IPv6
      atyp = 0x04;
      addrBytes = new Uint8Array(
        address.split(':').flatMap(x => [
  ...
  const request = new Uint8Array(4 + addrBytes.length + 2);
  request[0] = 0x05; // SOCKS5版本
...
function socks5AddressParser(address) {
	const match = REGEX.SOCKS5.exec(address);
	if (!match) {
...
const cmad = decodeURIComponent(atob(''))}
				</div>
...
async function 整理优选列表(api) {
	...
	try {
		const responses = await Promise.allSettled(
			api.map(apiUrl => NetworkUtils.fetchWithCache(apiUrl))
		);
		...
		ErrorHandler.log(error, '整理优选列表错误');
	}
...
function isValidIPv4(address) {
	return REGEX.IPV4.test(address);
}
...
async function makeReadableWebSocketStream(webSocket, earlyDataHeader, log) {
	return WebSocketUtils.createReadableStream(webSocket, earlyDataHeader, log);
}

function safeCloseWebSocket(socket) {
	WebSocketUtils.safeClose(socket);
}
